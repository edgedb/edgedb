...
type User extending NamedObject {
    ...
    optional multi link favorites -> NamedObject;
}
...
type ScalarTest {
    ...
    optional property p_decimal_str := <str>.p_decimal;
...
type BigIntTest {
    optional property value -> bigint;
}

# Inheritance tests
type Bar {
    optional property q -> str
}

type Bar2 extending Bar {
    optional property w -> str
}

type Rab {
    # target type will be overridden
    optional link blah -> Bar
}

type Rab2 extending Rab {
    overloaded optional link blah -> Bar2;
}
...
type Game extending NamedObject {
    optional multi link players -> User
}
