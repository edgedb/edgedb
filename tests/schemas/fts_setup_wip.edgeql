...
module default {
    ...
    type Comment extending Text {
        ...
        index fts::index on (
            fts::with_language(.comp_comment, fts::Language.English)
        );
    }
}
};
...
insert Quote {
    text := assert_single(
        (select Paragraph
         filter .number = 5 and .chapter.number = 1).text
    )
};

for P in (
    select Paragraph{sentence := re_match(r'^.+?\.', .text)[0]}
    filter len(.text) > len(.sentence)
) union (
    insert Quote {text := P.sentence}
);

# Comments that generally have nothing to do with the main text, this way it's
# easy to distinguish whether the hits are from `text` or `comp_comment`.
for x in enumerate({
    "These work and are a easy to set and empty. \
    I was really dubious when they arrived because \
    they don't seem very substantial but they got results.",

    "Amazing price for 20 mouse traps. When you buy traps online, \
    you have to look at the size carefully.",

    "Seems easy to use. Seems like a great value for the money.",

    "Wrong. I didn’t catch anything with these.",

    "We have had a lot of rain this year and so with all the plant \
    growth, it's paradise for rodents.",

    "The first box I got was ok, only a few didn't work.",

    "These are junk. The mice ate the food from the middle and the \
    trap never went off.",

    "Spring not strong enough.",

    "I got one the second night.",
    ...
    insert Comment {
        text := assert_single((
            ...
            filter Paragraph.number = x.0 + 5 and
                   Paragraph.chapter.number = 1
        )),
        comment := x.1,
    }
);