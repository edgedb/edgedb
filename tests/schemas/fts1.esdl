...
type Text {
    required text: str;
    index fts::index on (fts::with_options(.text, fts::Language.ISO_eng, 'A'));
}
...
type Post {
    ...
    index fts::index on ((
        fts::with_options(.title, fts::Language.ISO_eng, 'A'),
        fts::with_options(.body, fts::Language.ISO_eng, 'B')
    ));
}
...
type Description {
    ...
    index fts::index on (
        fts::with_options(.text, fts::Language.ISO_eng, 'A')
    );
}
