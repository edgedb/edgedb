...
class IRCompilerBase(ast.visitor.NodeVisitor,
    ...
    def visit_BinOp(self, expr):
        ...
        with self.context.new() as newctx:
            ...
            if is_bool_op and ctx.clause in {'result', 'where'}:
                # ORs and ANDs can form SQL implications that violate
                # the principle of existence of sets generated by path
                # expressions, so we need to consider all Path sets
                # in left and right branch for possible explicit
                # EXISTS() injection.
                #
                # See the comment in _apply_path_scope for details.
                with self.context.newsetscope() as scopectx:
                    left = self.visit(expr.left)
                    forced_setscope = set()
                    for s in scopectx.auto_setscope:
                        lax = scopectx.setscope.get(s)
                        if not lax:
                            forced_setscope.add(s)
            ...
            elif is_bool_op and ctx.clause in {'result', 'where'}:
                with self.context.newsetscope() as scopectx:
                    right = self.visit(expr.right)
                    for s in scopectx.auto_setscope:
                        lax = scopectx.setscope.get(s)
                        if lax:
                            forced_setscope.discard(s)
                        else:
                            forced_setscope.add(s)

                    newctx.forced_setscope.update(forced_setscope)
