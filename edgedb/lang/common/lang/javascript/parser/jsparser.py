...
class MissingToken(SyntaxError):
...
PREC = [
    ...
    ((UnaryRight,),
     ('PLUSPLUS', 'MINUSMINUS', 'PLUS', 'MINUS', 'BITNOT', 'NOT', 'DELETE', 'VOID', 'TYPEOF')),
    ...
    ((Assign,),
     ('ASSIGN', 'PLUSASSIGN', 'MINUSASSIGN', 'MULTASSIGN', 'DIVASSIGN', 'MODASSIGN',
      'LSHASSIGN', 'SRSHASSIGN', 'ZRSHASSIGN', 'ANDASSIGN', 'ORASSIGN'))
]
...
def nud(self):
    ...
    if self.parser.tentative_match('('):
        ...
        if not self.parser.tentative_match(')', regexp=False, consume=False):
    ...
    def get_next_token(self, regexp=True):
        ...
        if token == 'OP':
            ...
            if self.lexer.value in keywords.js_keywords:
                ...
                token = ('OP' if self.lexer.value in OP else
                         'VALUE' if self.lexer.value in ('null', 'this',
                                                         'true', 'false') else'KEYWORD')
    ...
    def parse_var_guts(self, statement=True):
        ...
        while True:
            ...
            if self.tentative_match('='):
                var_list.append(jsast.VarInitNode(name=varname.name,
