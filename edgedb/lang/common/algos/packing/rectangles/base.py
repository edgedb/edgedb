...
class Rectangle:
    def __init__(
            self, width, height, position: Point=None, *, data=None,
            children=None):
        self.width = width
        self.height = height
    ...
    def move(self, point: Point):
        ...
        return Rectangle(
            self.width, self.height, point, data=self.data,
            children=(c.move_relative(x_diff, y_diff) for c in self.children))

    def move_relative(self, x_diff, y_diff):
        return self.move(
            Point(self.position.x + x_diff, self.position.y + y_diff))

    def encloses(self, other):
        return (
            other.top_left.x >= self.top_left.x and
    ...
    def __eq__(self, other):  # should I take children into account?
        return (
            self.position == other.position and self.width == other.width and
    ...
    def __str__(self):
        return 'Rectangle(%sx%s at [%s, %s])' % (
            self.width, self.height, self.position.x, self.position.y)
