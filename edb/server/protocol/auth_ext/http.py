...
from .data import EmailFactor, OAuthError
...
class Router:
    ...
    async def handle_callback(
        ...
        if error is not None:
            ...
            params = OAuthError(error, error_description)
            logger.debug(f"OAuth provider returned an error: {params}")
            response.custom_headers["Location"] = util.join_url_params(
                redirect_to, dataclasses.asdict(params)
            )
            response.status = http.HTTPStatus.FOUND
            return
    ...
    async def handle_reset_password(
        ...
        try:
            ...
            logger.info(
                f"Reset password: identity_id={identity_id}, " f"pkce_id={code}"
            )
