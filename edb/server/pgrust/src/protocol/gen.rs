...
macro_rules! struct_elaborate {
    ...
    (__builder_type__ fixed($fixed:ident) fields([type(len)(len), value(), $($rest:tt)*] $($frest:tt)*) $($srest:tt)*) => {
        struct_elaborate!(__builder_docs__ fixed($fixed=>$fixed) fields([type($crate::protocol::meta::Length), size(fixed=fixed), value(auto=auto), $($rest)*] $($frest)*) $($srest)*);
    };
    (__builder_type__ fixed($fixed:ident) fields([type(len)(len), value($($value:tt)+), $($rest:tt)*] $($frest:tt)*) $($srest:tt)*) => {
        struct_elaborate!(__builder_docs__ fixed($fixed=>$fixed) fields([type($crate::protocol::meta::Length), size(fixed=fixed), value(value=($($value)*)), $($rest)*] $($frest)*) $($srest)*);
    };
    ...
    (__builder_value__ fixed($fixed:ident=>$fixed_new:ident) fields([
        ...
        struct_elaborate!(__builder_docs__ fixed($fixed=>$fixed_new) fields([type($ty), size($($size)*), value(no_value=no_value), $($rest)*] $($frest)*) $($srest)*);
    };
    (__builder_value__ fixed($fixed:ident=>$fixed_new:ident) fields([
        type($ty:ty), size($($size:tt)*), value($($value:tt)+), $($rest:tt)*
    ] $($frest:tt)*) $($srest:tt)*) => {
        struct_elaborate!(__builder_docs__ fixed($fixed=>$fixed_new) fields([type($ty), size($($size)*), value(value=($($value)*)), $($rest)*] $($frest)*) $($srest)*);
    };

    // Next, handle missing docs
    (__builder_docs__ fixed($fixed:ident=>$fixed_new:ident) fields([
        type($ty:ty), size($($size:tt)*), value($($value:tt)*), docs(), name($field:ident), $($rest:tt)*
    ] $($frest:tt)*) $($srest:tt)*) => {
        struct_elaborate!(__builder__ fixed($fixed=>$fixed_new) fields([type($ty), size($($size)*), value($($value)*), docs(concat!("`", stringify!($field), "` field.")), name($field), $($rest)*] $($frest)*) $($srest)*);
    };
    (__builder_docs__ fixed($fixed:ident=>$fixed_new:ident) fields([
        type($ty:ty), size($($size:tt)*), value($($value:tt)*), docs($($fdoc:literal)+), $($rest:tt)*
    ] $($frest:tt)*) $($srest:tt)*) => {
        struct_elaborate!(__builder__ fixed($fixed=>$fixed_new) fields([type($ty), size($($size)*), value($($value)*), docs(concat!($($fdoc)+)), $($rest)*] $($frest)*) $($srest)*);
    };
