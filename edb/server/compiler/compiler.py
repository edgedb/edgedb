    ...
    def _compile_ql_migration(
        ...
        if isinstance(ql, qlast.CreateMigration):
            ...
            if list(diff.get_subcommands()):
                debug.header('POPULATE MIGRATION mismatch')
                debug.dump(diff)
                raise errors.InternalServerError(
                    'POPULATE MIGRATION generated incorrect migration',
                    hint=(
                        'The migration generated by POPULATE MIGRATION does'
                        ' not produce the correct schema when applied.'
                        ' This is a bug.'
                    ),
                    context=ql.context,
                )
