...
class EdgeDatabaseStorageProviderInterface:
    ...
    def query_ids_for_a_type(self, tp: e.QualifiedName, filters: e.EdgeDatabaseSelectFilter) -> List[e.EdgeID]:
    ...
    def insert(self, id: e.EdgeID, tp: e.QualifiedName, props : Dict[str, e.MultiSetVal]) -> None:
        raise NotImplementedError()

    def delete(self, id: e.EdgeID, tp : e.QualifiedName) -> None:
        raise NotImplementedError()

    def update(self, id: e.EdgeID, tp: e.QualifiedName, props : Dict[str, e.MultiSetVal]) -> None:
        raise NotImplementedError()
    ...
    def project(self, id: e.EdgeID, tp: e.QualifiedName, property: str) -> e.MultiSetVal:
        raise NotImplementedError()
    ...
    def reverse_project(self, ids: Sequence[e.EdgeID], property: str) -> e.MultiSetVal:
        raise NotImplementedError()
...
class InMemoryEdgeDatabaseStorageProvider(EdgeDatabaseStorageProviderInterface):
    ...
    def query_ids_for_a_type(self, tp: e.QualifiedName, filters: e.EdgeDatabaseSelectFilter) -> List[e.EdgeID]:
        ...
        def check_filter_top(filter: e.EdgeDatabaseSelectFilter, id: e.EdgeID) -> bool:
            match filter:
                case e.EdgeDatabaseEqFilter(propname, arg):
                    return check_filter(e.EdgeDatabaseEqFilter(propname, arg), id)
    ...
    def project(self, id: e.EdgeID, tp: e.QualifiedName, prop: str) -> e.MultiSetVal:
    ...
    def reverse_project(self, subject_ids: Sequence[e.EdgeID], prop: str) -> e.MultiSetVal:
        results: List[e.Val] = []
        for (id, obj) in self.db.dbdata.items():
            if prop in obj.data.keys():
                object_vals = obj.data[prop].getVals()
                if all(isinstance(object_val, e.RefVal)
                    ...
                    for (object_id,
                        ...
                        if not all(isinstance(lbl, e.LinkPropLabel) for lbl in obj_linkprop_val.val.keys()):
...
class EdgeDatabase:
    ...
    def __init__(self, storage: EdgeDatabaseStorageProviderInterface) -> None:
        ...
        self.to_update : Dict[e.EdgeID, Tuple[e.QualifiedName, Dict[str, e.MultiSetVal]]]= {}
        self.to_insert = e.DB({})
    ...
    def insert(self, tp: e.QualifiedName, props : Dict[str, e.MultiSetVal]) -> e.EdgeID:
