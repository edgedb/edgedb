...
def do_write_logs(logs: List[Any], filename: str):
    ...
    #     result = """<a href='#' onclick='toggle("{}")'>Input/Output {}</a>\n""".format('_'.join(map(str, index)), '_'.join(map(str, index)))
    #     result += "<div class='entry' id='entry_{}'>".format('_'.join(map(str, index)))
    ...
    def format_entry(entry, index):
        ...
        result += "<button onclick='foldEntry(\"{}\")'>Fold</button> <button onclick='unfoldEntry(\"{}\")'>Unfold</button>".format(entry_id, entry_id)
        result += "<div class='entry' id='entry_{}'>".format(entry_id)
        result += "<div class='input'><span style='color:blue;'>Input:</span> {}</div>".format(entry[0])
        result += "<div class='input'><span style='color:green;'>Human-friendly Input:</span> {}</div>".format(codegen.generate_source(reverse_elab(entry[0])))
        if len(entry) > 1:
            result += "<div class='output'><span style='color:red;'>Output:</span> {}</div>".format(entry[1])
            result += "<div class='output'><span style='color:green;'>Human-friendly Output:</span> {}</div>".format(json.dumps(multi_set_val_to_json_like(entry[1]), indent=4))
        result += "</div>\n"
        return result


    def format_log(log, index):
        result = "<ul {} id='entry_{}'>\n".format("class='entry'" if len(index) > 0 else '', '_'.join(map(str, index)))
        for i, entry in enumerate(log):
            result += "<li>\n"
            if isinstance(entry, list):
                sub_index = index + (i,)
                result += """<a href='#' onclick='toggle("{}")'>Log {}</a>\n""".format('_'.join(map(str, sub_index)), '_'.join(map(str, sub_index)))
