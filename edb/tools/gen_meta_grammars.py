...
from edb.eschema.parser.grammar import keywords as esc_keywords
...
def main(names, con):
    ...
    operators = sorted(set(con.fetch(r'''
        WITH MODULE schema
    ...
    for gname in names:
        if gname == 'edgeql':
            ...
            code.extend(gen_grammar_class(
                ...
                collections.OrderedDict(
                    reserved_keywords=sorted(
                        esc_keywords.reserved_keywords - BOOL_LITERALS),
                    unreserved_keywords=sorted(
                        esc_keywords.unreserved_keywords - BOOL_LITERALS),
                    bool_literals=sorted(BOOL_LITERALS),
                    type_builtins=types,
                    module_builtins=sorted(s_schema.STD_MODULES),
                    constraint_builtins=constraints,
...
def gen_meta_grammars(names):
    ...
    NAME - one or both of: "edgeql", "eschema". The default is both.
    """
