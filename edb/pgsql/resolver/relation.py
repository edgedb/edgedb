...
def resolve_SelectStmt(
    ...
    if stmt.ctes:
        for cte in stmt.ctes:
            ...
            ctes.extend(get_top_level_ctes(ctx))
            ctes.append(cte)
    ...
    for clause in stmt.from_clause:
        ...
        # this is not really needed since FROM cannot contain DML,
        # but let's fulfill the promises of comments of this field.
        ctes.extend(get_top_level_ctes(ctx))
...
def get_top_level_ctes(
    ctx: context.ResolverContextLevel,
) -> Iterable[pgast.CommonTableExpr]:
