...
def pg_rebuild_all_pending_embeddings_views(
    ...
    for other_index in all_ai_indexes:
        if context and context.is_deleting(other_index):
            continue
...
def patch_fix_ai_indexes(
    ...
    for index in user_schema.get_objects(type=s_indexes.Index):
        ...
        commands.add_command(
            _pg_create_ai_embeddings_source_view(
                index, options, schema, context=None, rebuild_all=False)
        )

    commands.add_command(
        pg_rebuild_all_pending_embeddings_views(schema, context=None))
