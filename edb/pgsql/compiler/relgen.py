...
def process_func_as_fts_test(
    ...
    *,
    ...
    # In the common case where the language is a casted string
    # literal, strip off the ::text cast in favor of directly casting
    # the string to regconfig. Not doing this causes pg to fail to use
    # the index. (Well, this isn't really *that* much of a common case,
    # unless normalization is off. Which it will be inside a function, for
    # example.)
    match expr.args[0].expr:
        case irast.Set(expr=irast.TypeCast(
            expr=irast.Set(expr=irast.StringConstant(value=value))
        )):
