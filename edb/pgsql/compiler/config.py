...
def _rewrite_config_insert(
    ...
    overlays = ctx.env.rel_overlays[str(ir_set.typeref.material_type.id)]
    overlays.append(('replace', pgast.NullRelation(path_id=ir_set.path_id)))
    ...
    for el in ir_set.shape:
        if isinstance(el.expr, irast.InsertStmt):
            el.shape = list(filter(
                lambda e: e.rptr.ptrref.shortname.name != 'id',
                el.shape,
            ))
