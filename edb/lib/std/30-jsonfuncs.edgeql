...
{
    CREATE ANNOTATION std::description := 'Return the type of the outermost \
...
{
    CREATE ANNOTATION std::description := 'Return elements of JSON array as a \
        set of `json`.';
    SET volatility := 'IMMUTABLE';
...
{
    CREATE ANNOTATION std::description := 'Return set of key/value tuples \
        that make up the JSON object.';
    SET volatility := 'IMMUTABLE';
...
{
    CREATE ANNOTATION std::description := 'Return the JSON value at the end \
        of the specified path or an empty set.';
    SET volatility := 'IMMUTABLE';
    USING SQL $$
    SELECT COALESCE(
