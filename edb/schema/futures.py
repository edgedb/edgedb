...
class FutureCommandContext(
    sd.ObjectCommandContext[Future],
):
    pass
...
# generate a command.
_FutureHandler = Callable[
    ['FutureCommand', s_schema.Schema, sd.CommandContext, bool],
    tuple[s_schema.Schema, sd.Command],
]

FUTURE_HANDLERS: dict[str, _FutureHandler] = {}


def register_handler(name: str) -> Callable[
    [_FutureHandler], _FutureHandler
]:
    def func(f: _FutureHandler) -> _FutureHandler:
...
def future_enabled(schema: s_schema.Schema, feat: str) -> bool:
    return bool(schema.get_global(Future, feat, default=None))


class FutureCommand(
    sd.ObjectCommand[Future],
    context_class=FutureCommandContext,
):
    ...
    def copy(self: FutureCommand) -> FutureCommand:
        result = super().copy()
        if self.future_cmd:
    ...
    @classmethod
    def adapt(cls: Type[FutureCommand], obj: sd.Command) -> FutureCommand:
        result = super(FutureCommand, cls).adapt(obj)
        assert isinstance(obj, FutureCommand)
