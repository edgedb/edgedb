...
class InheritingObjectCommand(sd.ObjectCommand):
    ...
    def _alter_begin(self,
                     ...
                     scls: s_objtypes.ObjectType) -> s_schema.Schema:
        schema = super()._alter_begin(schema,  # type: ignore
                                      context,
                                      scls)
    ...
    def inherit_fields(
            ...
            schema: s_schema.Schema,
            context: sd.CommandContext,
            scls: s_objtypes.ObjectType,
            bases: Tuple[so.Object],
            *,
            fields: Optional[Iterable[str]] = None) -> s_schema.Schema:
        mcls = self.get_schema_metaclass()

        if fields is not None:
            field_names: Iterable[str] = set(scls.inheritable_fields()) & set(fields)
        else:
    ...
    def get_inherited_ref_layout(
            ...
            schema: s_schema.Schema,
            context: sd.CommandContext,
            refdict: so.RefDict) -> Dict[s_schema.Schema,
                Tuple[
    ...
    def get_no_longer_inherited_ref_layout(
        ...
        context: sd.CommandContext,
        refdict: so.RefDict,
        present_refs: Dict[s_schema.Schema,
                Tuple[
                    s_referencing.CreateReferencedObject,
                    qlast.ObjectDDL,
                    List[InheritingObject]
    ...
    def _recompute_inheritance(self,
        ...
        self.inherit_fields(schema, context, scls, bases)  # type: ignore
    ...
    def _reinherit_classref_dict(self,
                                 schema: s_schema.Schema,
                                 context: sd.CommandContext,
                                 refdict: so.RefDict) -> s_schema.Schema:
        from edb.schema import referencing as s_referencing
        ...
        for create_cmd, astnode, bases in refs.values():
            ...
            if obj is None:
                ...
                assert isinstance(obj, s_referencing.ReferencedInheritingObject)
    ...
    # TODO: is the following function never called?
    def _apply_rebase_ast(self,
                          context: sd.CommandContext,
...
def delta_bases(old_bases: Iterable[sn.SchemaName],
                new_bases: Iterable[sn.SchemaName]
                ) -> Tuple[Tuple[so.ObjectRef, ...],
                           Tuple[List[Tuple[List[so.ObjectRef],
                            Union[str, so.ObjectRef,
                                  Tuple[str, so.ObjectRef]]]], ...]]:
    ...
    added_bases: List[Tuple[List[so.ObjectRef],
                            Union[str, so.ObjectRef,
                                  Tuple[str, so.ObjectRef]]]] = []
...
class AlterInherit(sd.Command):
    ...
    def _cmd_tree_from_ast(cls,
                           schema: s_schema.Schema,
                           astnode: qlast.AlterDropInherit,
...
class CreateInheritingObject(InheritingObjectCommand, sd.CreateObject):
    ...
    def _create_begin(self,
        ...
        if not context.canonical:
            ...
            if context.inheritance_merge is None or context.inheritance_merge:
                schema = self.inherit_fields(schema,
                                             context,
                                             self.scls,
                                             bases)
    ...
    def _cmd_tree_from_ast(cls,
        ...
        assert isinstance(cmd, s_schema.Schema)
        return cmd
    ...
    def inherit_classref_dict(self,
                              schema: s_schema.Schema,
                              context: sd.CommandContext,
                              refdict: so.RefDict
                              ) -> sd.CommandGroup:
...
class AlterInheritingObject(InheritingObjectCommand, sd.AlterObject):
    ...
    def _cmd_tree_from_ast(cls,
        ...
        assert isinstance(cmd, s_objtypes.AlterObjectType)
    ...
    def _propagate_field_alter(self,
                               schema: s_schema.Schema,
                               context: sd.CommandContext,
                               scls: s_objtypes.ObjectType,
...
class RebaseInheritingObject(AlterInheritingObjectFragment):
    ...
    def apply(self,
        ...
        assert isinstance(scls, s_objtypes.ObjectType)
...
class InheritingObject(derivable.DerivableObject):
    ...
    def delta(cls,
              ...
              *,
              ...
              new_schema: s_schema.Schema) -> sd.ObjectCommand:
        if context is None:
    ...
    def inheritable_fields(self) -> Iterable[Any]:  # TODO: what is Any here?
        for fn, f in self.__class__.get_fields().items():
            if f.inheritable:
                yield fn

    def get_base_names(self,
                       schema: s_schema.Schema) -> Tuple[sn.SchemaName, ...]:
        return self.get_bases(schema).names(schema)

    def get_topmost_concrete_base(self,
                                  schema: s_schema.Schema) -> InheritingObject:
    ...
    @classmethod
    def get_default_base_name(self) -> None:
        return None
