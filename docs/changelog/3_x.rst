...
If you're still running pre-1.0 EdgeDB instances (e.g., 1.0-beta3) and want
to upgrade to 3.0, you would need a dump/restore to version 2.x first,
followed by another dump/restore to 3.0.
...
Query planning is available in the CLI REPL by prepending your query with
...
* Add the ``edgedb migration update-check`` command
...
recieve support for a year and a half after their initial release.
