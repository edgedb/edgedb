...
``edgedb cli upgrade``. Then run an upgrade check to make sure your schema will
migrate cleanly to 3.0.
...
RC 4 is a re-release of RC 3 fixing a packaging problem that resulted in
non-portable binaries which would crash on systems with older CPUs.
This version also includes a fix for ``pgvector`` data support in the Web UI.
