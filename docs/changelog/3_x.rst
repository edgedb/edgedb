...
.. code-block:: bash
  ...
  $ edgedb project init --server-version=3.0-rc.4
...
1. Spin up an empty 3.0 instance. You can use one of our :ref:`deployment
   ...
   .. code-block:: bash
       ...
       $ sudo apt-get update && sudo apt-get install edgedb-3-rc4

   Under CentOS/RHEL, use this installation command:

   .. code-block:: bash

       $ sudo yum install edgedb-3-rc4

   In any required ``systemctl`` commands, replace ``edgedb-server-2`` with
   ``edgedb-server-3``.

   Under any Docker setups, supply the ``3.0-rc.4`` tag.
