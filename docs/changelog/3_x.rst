...
* Support casting a UUID to a type (:edb:gh:`#4469`). This is a handy way to
...
* Add the :eql:func:`json_object_pack` function to construct JSON from an array
  of key/value tuples. (:edb:gh:`#4474`)
...
* Support tuples as query arguments (:edb:gh:`#4489`)
...
* Add the syntax for abstract indexes (:edb:gh:`#4691`)
...
* Implement migration rewrites (:edb:gh:`#4585`)

* Implement schema reset (:edb:gh:`#4714`)

* Support link properties on computed backlinks (:edb:gh:`#5227`)
...
* Fix crash on cycle between defaults in insert (:edb:gh:`#5355`)

* Improvements to top-level server error reporting (:edb:gh:`#5349`)

* Forbid ranges of user-defined scalars (:edb:gh:`#5345`)

* Forbid DML in non-scalar function args (:edb:gh:`#5310`)

* Don't let "owned" affect how we calculate backlinks (:edb:gh:`#5306`)

* Require inheritance order to be consistent with the specified base order
  (:edb:gh:`#5276`)

* Support using non-strict functions in simple expressions (:edb:gh:`#5271`)
...
* Fix 3-way unions of certain types with policies (:edb:gh:`#5205`)

* Fix simultaneous deletion of objects related by multi links (:edb:gh:`#5201`)

* Respect ``enforce_access_policies := false``
  inside functions (:edb:gh:`#5199`)
...
* Make uuidgen properly set versions in uuid4/uuid5 (:edb:gh:`#5188`)

* Disallow variadic arguments with optional types in user code.
  (:edb:gh:`#5110`)

* Support casting between scalars with a common concrete base (:edb:gh:`#5108`)

* Fix GROUP regression with some query-builder queries (:edb:gh:`#5071`)
...
* Fix non-transactional errors in Postgres 14.7 (:edb:gh:`#5028`)

* Properly cast to containers of enums when loading from the schema
  (:edb:gh:`#4988`)

* Implement manual error override configuration (:edb:gh:`#4974`)

* Fix protocol state confusion after rollback
  (:edb:gh:`#4970`), (:edb:gh:`#4953`)
