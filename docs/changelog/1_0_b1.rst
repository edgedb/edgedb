...
* Make the format of :eql:type:`duration` less ambiguous by
  restricting the usage of ``-`` sign (:edb:gh:`#2229`).
* Record non-DDL commands during migrations instead of executing them
  immediately as per `RFC 1000 <migrations_>`_ (:edb:gh:`#2138`).
* Add more details to the DDL command status (:edb:gh:`#2138`).
...
* Stop using ``drop`` to change field value, introduce ``reset`` and
  ``set`` syntax to do that (:edb:gh:`#2031`).
...
* ``alter ... set type`` now requires an explicit conversion
  ...
  assignment-castable from the old type (:edb:gh:`#2115`).
...
* Add a ``using`` clause for ``set required`` so that en expression to
  fill in missing values can be specified (:edb:gh:`#2130`).
...
* Drop ``is_`` prefixes from boolean fields in introspection schema.
  ...
  deprecated later (:edb:gh:`#1793`).
* Add support for computed link properties (:edb:gh:`#2067`).
* Infer and validate volatility for functions (:edb:gh:`#1937`).
* Allow trailing commas in functions (:edb:gh:`#1462`).
...
* Fix issues with :eql:func:`enumerate` when applied to objects
  (:edb:gh:`#1815`) and results of function calls (:edb:gh:`#1816`).
* Fix ``drop property`` for ``multi`` properties (:edb:gh:`#2059`).
...
* Fix accessing links on objects that come from functions and other
  sources that aren't simple paths (:edb:gh:`#1887`).
