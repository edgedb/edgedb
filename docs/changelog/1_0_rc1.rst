...
* Always escape newlines when string literals appear in automatically
  generated code, such as in migrations (:edb:gh:`#2704`).
* Fix behavior of ``set multi`` to make sure that it does not imply
  ``set required`` (:edb:gh:`#2778`).
* Make properties non-distinct by default (:edb:gh:`#2853`).
...
* Make computed links and properties defined in the schema behave the
  ...
  ad-hoc directly in the query (:edb:gh:`#2558`).


EdgeQL
======

* Drop ``final`` from the syntax (:edb:gh:`#2607`).
* Restore the precedence of :eql:op:`detached` to no longer match that
  of :eql:op:`exists` undoing the change :edb:gh:`#2638`.
* Fix ``unless conflict`` with inheritance hierarchies (:edb:gh:`#2525`).
* Always use serializable isolation (:edb:gh:`#2877`).
* Correctly raise exclusive constraints when performing an ``insert``
  or an ``update`` on related types (:edb:gh:`#2845`).
* Enforce link or property cardinality in mutations (:edb:gh:`#2827`).
...
* Implement :eql:func:`assert_exists` which allows to perform a
  cardinality check in run-time (:edb:gh:`#2901`):
...
* Fix deletion issues for multi properties (:edb:gh:`#2883`).

* Disallow some broken DML/volatile interactions (:edb:gh:`#2811`).
...
* Fix a regression when objects from opaque sources are passed to
  functions (:edb:gh:`#2924`).

* Fix accessing link properties on union types (:edb:gh:`#2513`).
* Fix some introspection issues with backlinks (:edb:gh:`#2599`)
* Fix bare backlink schema-defined computed link (:edb:gh:`#2605`).
* Make filtered backlinks work even when there exists a computed
  backlink (:edb:gh:`#2698`).
...
* Make SCRAM the default auth method (:edb:gh:`#2848`).
