...
* Support mutation in ``USING`` expressions when changing a link to ``required``
  or to ``single`` during a migration
  (:eql:gh:`#4873`)
...
* Fix non-transactional commands like ``DROP DATABASE`` when using Postgres 14.7
  (:eql:gh:`#5026`)
