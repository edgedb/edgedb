...
:edb-alt-title: EdgeDB v2 — Sagittarius | Changelog

.. warning::

    This document is a work in progress and will keep being updated
    until the `v2 release <https://lu.ma/edgedb>`_ on July 28th 2022.
...
``edgedb ui`` inside any :ref:`EdgeDB project <ref_guide_using_projects>`, or specify a local instance name with ``edgedb ui -I my_inst``. The
...
<ref_eql_free_objects>` representing each group, including the grouping, the
grouping *key*, and the set of elements.
...
Object types can now be augmented with object-level access policies. When combined with global variables, access policies can be used to
...
Refer to :ref:`the docs <ref_datamodel_access_policies>` or `RFC 1011 <acls_>`_ for full details.


Range types
-----------

EdgeDB now supports :ref:`range types <ref_std_range>` representing intervals of values.

.. code-block:: edgeqpl-repl
...
This release also introduces a new datatype :eql:type:`cal::date_duration` to represent a span of *months/days*. It is nearly equivalent to the existing :eql:type:`cal::relative_duration` but cannot represent sub-day durations.
...
* Support additional arithmetic operations on local date and time types, including :eql:func:`duration_get`, :eql:func:`cal::duration_normalize_hours`, and :eql:func:`cal::duration_normalize_days`. Per `RFC 1013 <dates_>`_.
...
* Support user-provided values for the ``id`` property when inserting objects
  ...
  .. code-block::
    ...
    insert User {
      id := "5abf67cc-9f9f-4bbc-b009-d117d463a12e",
