...
* Implement casts between JSON and enums, arrays and tuples
  (:edb:gh:`#251`). For example, now there's a way to unpack JSON
...
* Allow several mutation operations in a single mutation query
  (:edb:gh:`#1569`).
* Reflect nested aliased types (:edb:gh:`#722`).
* Enable sorting on non-trivial path (:edb:gh:`#1642`). Here's an
...
* Add an ``exists`` filter operation (:edb:gh:`#1655`). Here's an
